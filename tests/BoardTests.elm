...
---
"""
                            stones = (Dict.fromList [
                                                    ((1, 2), Player.Black)
                                                    , ((2,1), Player.Black)
                                                    ])
                            captures = ([(1, 1)], [])
                            expectedBoard = (Board.fromSizeStonesCaptures 3 stones captures)
                        in
                        case board of
                            Err _ -> Expect.fail "need a valid board"
                            Ok board_ ->
                                case Board.capture board_ (Move.fromPlayerAndPosition Player.Black (2, 1)) of
                                    Err _ -> Expect.fail "this should succeed"

                                    Ok board__ -> Expect.equal board__ expectedBoard
